...
3. **Build** — invoke the `develop` skill (type `/develop <feature-slug>`, or ask to
   "build/implement <feature-slug>"). It implements the Technical Spec exactly, then validates (check-only) → deploys to
   the sandbox → runs Apex tests, and appends everything to the implementation log.
